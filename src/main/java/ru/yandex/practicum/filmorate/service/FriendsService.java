...
public class FriendsService {
    ...
    public Friends addFriends(long idUser, long idUserFriend) {
        return friendsStorage.getFriendsConnectionsTwoUsers(idUser, idUserFriend).orElseGet(
                () -> friendsStorage.addFriends(idUser, idUserFriend)
                        .orElseThrow(() -> new InternalServerException(String
    ...
    public List<Friends> getCommonFriends(Long idUser, Long idUserFriend) {
        return friendsStorage.getFriendsById(idUser)
                .stream()
                .filter(friend -> friendsStorage.getFriendsConnectionsTwoUsers(
                        idUserFriend, friend.getIdFriendUser()).isPresent())
                .toList();
    }
}
